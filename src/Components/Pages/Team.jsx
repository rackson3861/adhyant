import React from "react";

export default function Team() {
  return (
    <>
      <div className="container-xxl py-5">
        <div className="container">
          <div className="text-center wow fadeInUp" data-wow-delay="0.1s">
            <h6 className="section-title bg-white text-center text-primary px-3">
              Team
            </h6>
          <h1 className="mb-5">Expert Faculties</h1>
          </div>
          <div className="row g-4 justify-content-center">
            <div
              className="col-lg-3 col-md-6 col-6 wow fadeInUp"
              data-wow-delay="0.1s"
            >
              <div className="team-item" style={{ backgroundColor: "var(--service-bg)" }}>
                <div className="overflow-hidden">
                  <img className="img-fluid" src="/img/Team/Awanish.svg" alt="" />
                </div>
                <div
                  className="position-relative d-flex justify-content-center"
                  style={{ marginTop: "-23px" }}
                >
                  <div className="d-flex justify-content-center pt-2 px-1" style={{ backgroundColor: "var(--service-bg)" }}>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-facebook-f" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-twitter" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-instagram" />
                    </a>
                  </div>
                </div>
                <div className="text-center p-4">
                  <h5 className="mb-0">Awanish Jha</h5>
                  <small><b>IIT Guwahati</b></small> <br/>
                  <small>Physic Faculty</small>
                </div>
              </div>
            </div>
            <div
              className="col-lg-3 col-md-6 col-6 wow fadeInUp"
              data-wow-delay="0.3s"
            >
              <div className="team-item" style={{ backgroundColor: "var(--service-bg)" }}>
                <div className="overflow-hidden">
                  <img className="img-fluid" src="/img/Team/Pooja.svg" alt="" />
                </div>
                <div
                  className="position-relative d-flex justify-content-center"
                  style={{ marginTop: "-23px" }}
                >
                  <div className="d-flex justify-content-center pt-2 px-1" style={{ backgroundColor: "var(--service-bg)" }}>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-facebook-f" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-twitter" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-instagram" />
                    </a>
                  </div>
                </div>
                <div className="text-center p-4">
                  <h5 className="mb-0">Pooja Gupta</h5>
                  <small><b>IIT Guwahati</b></small><br/>
                  <small>Chemistry Faculty</small>
                </div>
              </div>
            </div>
            <div
              className="col-lg-3 col-md-6 col-6 wow fadeInUp"
              data-wow-delay="0.5s"
            >
              <div className="team-item" style={{ backgroundColor: "var(--service-bg)" }}>
                <div className="overflow-hidden">
                  <img className="img-fluid" src="/img/Team/Uttam.svg" alt="" />
                </div>
                <div
                  className="position-relative d-flex justify-content-center"
                  style={{ marginTop: "-23px" }}
                >
                  <div className="d-flex justify-content-center pt-2 px-1" style={{ backgroundColor: "var(--service-bg)" }}>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-facebook-f" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-twitter" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-instagram" />
                    </a>
                  </div>
                </div>
                <div className="text-center p-4">
                  <h5 className="mb-0">Uttam Singh</h5>
                  <small><b>IIT Guwahati</b></small><br/>
                  <small>Maths Faculty</small>
                </div>
              </div>
            </div>
            <div
              className="col-lg-3 col-md-6 col-6 wow fadeInUp"
              data-wow-delay="0.7s"
            >
              <div className="team-item" style={{ backgroundColor: "var(--service-bg)" }}>
                <div className="overflow-hidden">
                  <img className="img-fluid" src="/img/Team/Renuka.svg" alt="" />
                </div>
                <div
                  className="position-relative d-flex justify-content-center"
                  style={{ marginTop: "-23px" }}
                >
                  <div className="d-flex justify-content-center pt-2 px-1" style={{ backgroundColor: "var(--service-bg)" }}>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-facebook-f" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-twitter" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-instagram" />
                    </a>
                  </div>
                </div>
                <div className="text-center p-4">
                <h5 className="mb-0">Renuka Yadav</h5>
                  <small><b>NIT Kurushetra</b></small><br/>
                  <small>Maths Faculty</small>
                </div>
              </div>
            </div>
          </div>
          
          <div className="text-center wow fadeInUp" data-wow-delay="0.1s">
            <br/>
            <h1 className="mb-5">Expert Mentors</h1>
          </div>
          
          <div className="row g-4 justify-content-center">
            <div
              className="col-lg-3 col-md-6 col-6 wow fadeInUp"
              data-wow-delay="0.1s"
            >
              <div className="team-item" style={{ backgroundColor: "var(--service-bg)" }}>
                <div className="overflow-hidden">
                  <img className="img-fluid" src="/img/Team/Deepak.svg" alt="" />
                </div>
                <div
                  className="position-relative d-flex justify-content-center"
                  style={{ marginTop: "-23px" }}
                >
                  <div className="d-flex justify-content-center pt-2 px-1" style={{ backgroundColor: "var(--service-bg)" }}>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-facebook-f" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-twitter" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-instagram" />
                    </a>
                  </div>
                </div>
                <div className="text-center p-4">
                  <h5 className="mb-0">Deepak Gupta</h5>
                  <small><b>NIT Allahabad</b></small>
                  <br/>
                  <small>Chief Advisor</small>
                </div>
              </div>
            </div>
            <div
              className="col-lg-3 col-md-6 col-6 wow fadeInUp"
              data-wow-delay="0.3s"
            >
              <div className="team-item" style={{ backgroundColor: "var(--service-bg)" }}>
                <div className="overflow-hidden">
                  <img className="img-fluid" src="/img/Team/Arti.svg" alt="" />
                </div>
                <div
                  className="position-relative d-flex justify-content-center"
                  style={{ marginTop: "-23px" }}
                >
                  <div className="d-flex justify-content-center pt-2 px-1" style={{ backgroundColor: "var(--service-bg)" }}>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-facebook-f" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-twitter" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-instagram" />
                    </a>
                  </div>
                </div>
                <div className="text-center p-4">
                  <h5 className="mb-0">Arti Gupta</h5>
                  <small><b>IIT Dhanbad</b></small><br/>
                  <small>Chief Academic Officer</small>
                </div>
              </div>
            </div>
            <div
              className="col-lg-3 col-md-6 col-6 wow fadeInUp"
              data-wow-delay="0.5s"
            >
              <div className="team-item" style={{ backgroundColor: "var(--service-bg)" }}>
                <div className="overflow-hidden">
                  <img className="img-fluid" src="/img/Team/Sumit.svg" alt="" />
                </div>
                <div
                  className="position-relative d-flex justify-content-center"
                  style={{ marginTop: "-23px" }}
                >
                  <div className="d-flex justify-content-center pt-2 px-1" style={{ backgroundColor: "var(--service-bg)" }}>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-facebook-f" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-twitter" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-instagram" />
                    </a>
                  </div>
                </div>
                <div className="text-center p-4">
                  <h5 className="mb-0">Sumit Rai</h5>
                  <small><b>IIT Dhanbad</b></small><br/>
                  <small>Chief Technical Officer</small>
                </div>
              </div>
            </div>
            <div
              className="col-lg-3 col-md-6 col-6 wow fadeInUp"
              data-wow-delay="0.7s"
            >
              <div className="team-item" style={{ backgroundColor: "var(--service-bg)" }}>
                <div className="overflow-hidden">
                  <img className="img-fluid" src="/img/Team/Akansha.svg" alt="" />
                </div>
                <div
                  className="position-relative d-flex justify-content-center"
                  style={{ marginTop: "-23px" }}
                >
                  <div className="d-flex justify-content-center pt-2 px-1" style={{ backgroundColor: "var(--service-bg)" }}>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-facebook-f" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-twitter" />
                    </a>
                    <a className="btn btn-sm-square btn-primary mx-1" href>
                      <i className="fab fa-instagram" />
                    </a>
                  </div>
                </div>
                <div className="text-center p-4">
                  <h5 className="mb-0">Akanksha Gupta</h5>
                  <small><b>RTU</b></small>
                  <br/>
                  <small>Chief Marketing Officer</small>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
